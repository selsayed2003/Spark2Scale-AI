...
class InlineHTMLToRunsParser(HTMLParser):
    def __init__(self, base_font: PptxFontModel, highlight_font: Optional[PptxFontModel] = None):
        super().__init__(convert_charrefs=True)
        self.base_font = base_font
        self.highlight_font = highlight_font  # For <b>: accent color + bold (premium emphasis)
    ...
    def _current_font(self) -> PptxFontModel:
        ...
        if is_bold and self.highlight_font:
            return self.highlight_font
...
def parse_html_text_to_text_runs(
    ...
    parser = InlineHTMLToRunsParser(
        base_font if base_font else PptxFontModel(),
